...
# ─────────────────────────────────────────────────────────────────────────────
# PRECOMPUTED INDICATORS (backtest performance optimisation)
# ─────────────────────────────────────────────────────────────────────────────

class PrecomputedIndicators:
    """
    Wraps a full DataFrame so the backtest engine can call .at(i) for each
    bar without changing its API.  This version delegates to calculate_all()
    on a 500-bar lookback slice — identical results to the non-precomputed
    path, just with the engine-facing interface the optimised engine expects.
    """

    LOOKBACK = 500  # bars of history passed to calculate_all

    def __init__(self, df: pd.DataFrame) -> None:
        self._df = df.reset_index(drop=False)   # keep index as column for slicing

    def at(self, i: int) -> AllIndicators:
        """Return AllIndicators for bar index i."""
        start = max(0, i - self.LOOKBACK + 1)
        slice_df = self._df.iloc[start : i + 1].set_index(self._df.columns[0])
        return calculate_all(slice_df)
